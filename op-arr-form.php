...
if ($id) {
	$row = DB::queryFirstRow("SELECT * FROM arrival WHERE id=%s", $id);
	$q_apa = u;
} else{
	$row = array();
	$q_apa = c; /* create */
...
<title>FIDS: arrival form</title>
</head>
<body>
<div class="padin">
<form action="op-arr-save.php" method="post" class="fform">
...
<p><label>Origin: </label>
   <span><select name="origin" required class="ori"><option value=''></option><?php
		include("list-fav-city-nv.txt");
		foreach ($listcity as $lcity) {
			echo "<option value='".$lcity['apcode']."'";
                        if($row['origin'] == $lcity['apcode']){ echo " selected"; }
...
<p><label>Airline: </label>
   <span><select class="al" name="airline" required><option value='0'></option><?php
			include("list-fav-airline.txt");
			foreach ($listair as $lair) {
				echo "<option value='".$lair['alcode']."'";
...
<p><label>Sched. Arrival: </label>
   <span><input type="text" autocomplete="off" name="schedarrival" id="schedpicker" class="timepicker" value="<?= substr($row['schedarrival'],0,16)?>" required></span></p>
